...
**扩展文档（Open-Agent-Teams）**：[docs/open-agent-teams/README.md](docs/open-agent-teams/README.md) — Open Multi-Agent + Hermes 私有化集成模板与规格索引（与 monorepo 业务包无强耦合）。**Node 工作区**：根目录 `pnpm-workspace.yaml`，包见 `packages/openclaw`、`packages/hermes-agent` 与 [monorepo-packages.md](docs/open-agent-teams/monorepo-packages.md)。
