...
export function createDocumentToolsV2() {
    return [
        ...
        defineTool({
            ...
            description: '创建可沉淀的文档，支持项目/任务/Agent关联。' +
        ...
        defineTool({
            ...
            execute: async (input) => {
                try {
                    ...
                    const summary = result.documents.map(d => `- [${d.type}] ${d.title} | ${d.authorName} | ${new Date(d.updatedAt).toLocaleDateString()} (${d.commentCount}评论)`).join('\n');
        ...
        defineTool({
            ...
            execute: async (input) => {
                try {
                    ...
                    const summary = docs.map(d => `- [${d.type}] ${d.title} | ${new Date(d.updatedAt).toLocaleDateString()} | ${d.commentCount}评论`).join('\n');
        ...
        defineTool({
            ...
            execute: async (input) => {
                try {
                    ...
                    const summary = comments.map(c => `${c.resolved ? '✅' : '💬'} ${c.authorName}: ${c.content.substring(0, 50)}${c.content.length > 50 ? '...' : ''}`).join('\n');
        ...
        defineTool({
            ...
            execute: async (input) => {
                try {
                    ...
                    const summary = versions.map(v => `- v${v.version}: ${v.title} | ${v.authorId} | ${new Date(v.createdAt).toLocaleDateString()}`).join('\n');
...
//# sourceMappingURL=document-tools-v2.js.map