...
export interface DocumentV2 {
  ...
  tags: string[];
  relatedDocIds: string[]; // 关联文档
  relatedTaskIds: string[]; // 关联任务
  relatedAgentIds: string[]; // 关联 Agent
  commentCount: number;
  createdAt: number;
  updatedAt: number;
...
export class DocumentManager {
  ...
  createProject(name: string, description?: string): Project {
    ...
    const now = Date.now();
    const project: Project = { id, name, description: description || '', createdAt: now, updatedAt: now };
  ...
  createTask(projectId: string, title: string, description?: string, assignee?: string): Task {
    ...
    const now = Date.now();
    const task: Task = { id, projectId, title, description: description || '', status: 'todo', assignee: assignee || '', createdAt: now, updatedAt: now };
  ...
  createDocument(doc: Omit<DocumentV2, 'id' | 'createdAt' | 'updatedAt' | 'version' | 'commentCount'>): DocumentV2 {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id, fullDoc.title, fullDoc.content, fullDoc.type, fullDoc.projectId || null, fullDoc.taskId || null,
      fullDoc.authorId, fullDoc.authorName, fullDoc.version, fullDoc.parentId || null,
      JSON.stringify(fullDoc.tags), JSON.stringify(fullDoc.relatedDocIds),
  ...
  getDocument(id: string): DocumentV2 | null {
    const row = this.db.prepare('SELECT * FROM documents_v2 WHERE id = ?').get(id) as any;
    if (!row) return null;
  ...
  updateDocument(id: string, updates: Partial<Omit<DocumentV2, 'id' | 'createdAt' | 'version' | 'commentCount'>>, authorId?: string, authorName?: string): DocumentV2 | null {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
      `ver-${Date.now()}`, id, existing.version, existing.title, existing.content,
      authorId || existing.authorId, Date.now()
    );

    const updated: DocumentV2 = {
      ...existing,
      ...updates,
      version: newVersion,
      updatedAt: Date.now(),
    ...
    this.db.prepare(`
      ...
      WHERE id = ?
    `).run(
      updates.title ?? null, updates.content ?? null, updates.type ?? null,
      updates.projectId ?? null, updates.taskId ?? null,
      authorId || existing.authorId, authorName || existing.authorName, newVersion,
      updates.parentId ?? null,
      updates.tags ? JSON.stringify(updates.tags) : null,
      updates.relatedDocIds ? JSON.stringify(updates.relatedDocIds) : null,
      updates.relatedTaskIds ? JSON.stringify(updates.relatedTaskIds) : null,
      updates.relatedAgentIds ? JSON.stringify(updates.relatedAgentIds) : null,
      updated.updatedAt,
  ...
  queryDocuments(query: DocumentQuery): { documents: DocumentV2[]; total: number } {
    ...
    // 总数
    const totalRow = this.db.prepare(`SELECT COUNT(*) as count FROM documents_v2 WHERE ${whereClause}`).get(...params) as any;
   ...
   */
  getDocumentsByAgent(agentId: string, options?: { projectId?: string; type?: string }): DocumentV2[] {
  ...
  addComment(comment: Omit<DocumentComment, 'id' | 'createdAt'>): DocumentComment {
    ...
    const now = Date.now();
    const fullComment: DocumentComment = { ...comment, id, createdAt: now };
  ...
  getVersions(documentId: string): Array<{ id: string; version: number; title: string; authorId: string; createdAt: number }> {
    const rows = this.db.prepare(`
  ...
  stats(): {
    ...
    const totalDocuments = (this.db.prepare('SELECT COUNT(*) as count FROM documents_v2').get() as any).count;
    ...
    const totalComments = (this.db.prepare('SELECT COUNT(*) as count FROM document_comments').get() as any).count;

    const typeRows = this.db.prepare('SELECT type, COUNT(*) as count FROM documents_v2 GROUP BY type').all() as any[];
    const typeDistribution: Record<string, number> = {};
    for (const row of typeRows) typeDistribution[row.type] = row.count;

    const authorRows = this.db.prepare('SELECT author_id, COUNT(*) as count FROM documents_v2 GROUP BY author_id').all() as any[];
    const authorDistribution: Record<string, number> = {};
    for (const row of authorRows) authorDistribution[row.author_id] = row.count;

    const recentRows = this.db.prepare('SELECT * FROM documents_v2 ORDER BY updated_at DESC LIMIT 5').all() as any[];
  ...
  private rowToDocument(row: any): DocumentV2 {
    return {
  ...
  private rowToComment(row: any): DocumentComment {
    return {
...
let globalDocumentManager: DocumentManager | null = null;

export function getGlobalDocumentManager(config?: DocumentManagerConfig): DocumentManager {
...
export function resetGlobalDocumentManager(): void {
