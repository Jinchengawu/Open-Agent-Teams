...
export interface KnowledgeDocument {
  ...
  source: string;        // 来源 Agent ID 或 Pipeline 面 ID
  tags: string[];
  createdAt: number;
  updatedAt: number;
...
export class KnowledgeCenter {
   ...
   * 添加文档到知识中心
   */
  addDocument(doc: Omit<KnowledgeDocument, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): KnowledgeDocument {
    ...
    stmt.run(
      ...
      fullDoc.type,
      fullDoc.source,
      JSON.stringify(fullDoc.tags),
      fullDoc.createdAt,
      fullDoc.updatedAt,
      JSON.stringify(fullDoc.metadata || {})
    );
   ...
   * 列出所有文档（支持分页）
   */
  listDocuments(options?: { type?: string; source?: string; limit?: number; offset?: number }): KnowledgeDocument[] {
  ...
  search(query: KnowledgeQuery): KnowledgeResult[] {
    const results: Map<string, KnowledgeResult> = new Map();
   ...
   * 自然语言查询（带上下文摘要）
   */
  async query(question: string, options?: { limit?: number }): Promise<{ answer: string; sources: KnowledgeResult[] }> {
   ...
   * FTS5 全文搜索
   */
  private searchFTS(query: string): KnowledgeResult[] {
    try {
      // 清理查询：去除非法字符，支持中文和英文
      ...
      const rows = this.db.prepare(`
        ...
        ORDER BY rank
        LIMIT ?
      `).all(ftsQuery, this.maxResults) as any[];
   ...
   */
  private searchSemantic(query: string): KnowledgeResult[] {
   ...
   * 标签搜索
   */
  private searchByTags(tags: string[]): KnowledgeResult[] {
    ...
    const rows = this.db.prepare(`
      ...
      LIMIT ?
    `).all(...tags, this.maxResults) as any[];
   ...
   * 来源搜索
   */
  private searchBySource(source: string): KnowledgeResult[] {
    const rows = this.db.prepare(`
      SELECT * FROM documents WHERE source = ? ORDER BY updated_at DESC LIMIT ?
    `).all(source, this.maxResults) as any[];
   ...
   * 统计信息
   */
  stats(): { total: number; types: Record<string, number>; sources: Record<string, number> } {
    const total = (this.db.prepare('SELECT COUNT(*) as count FROM documents').get() as any).count;
    
    const typeRows = this.db.prepare('SELECT type, COUNT(*) as count FROM documents GROUP BY type').all() as any[];
    const types: Record<string, number> = {};
    for (const row of typeRows) {
      types[row.type] = row.count;
    }

    const sourceRows = this.db.prepare('SELECT source, COUNT(*) as count FROM documents GROUP BY source').all() as any[];
...
let globalKnowledgeCenter: KnowledgeCenter | null = null;

export function getGlobalKnowledgeCenter(config?: KnowledgeCenterConfig): KnowledgeCenter {
...
export function resetGlobalKnowledgeCenter(): void {
  globalKnowledgeCenter?.close();
  globalKnowledgeCenter = null;
}

export function createKnowledgeCenter(config?: KnowledgeCenterConfig): KnowledgeCenter {
