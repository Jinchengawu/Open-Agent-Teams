...
export class ConflictResolver {
  ...
  private defaultStrategy: ConflictStrategy = 'arbitration';
   ...
   * 策略 3: 合并策略（取并集）
   */
  private merge(conflict: Conflict): ConflictResolution {
   ...
   * 策略 4: 最新优先
   */
  private latest(conflict: Conflict): ConflictResolution {
    // 按完成时间排序，取最新的
    let latest: string = conflict.surfaceIds[0];
    let latestTime = 0;
   ...
   * 策略 5: 人工介入（暂停等待）
   */
  private async manual(conflict: Conflict): Promise<ConflictResolution> {
    // 发布事件，等待人工介入
...
${JSON.stringify(
  Object.fromEntries(
    Object.entries(conflict.artifacts).map(([k, v]) => [
      k,
...
`;
   ...
   * 构建投票提示词
   */
  private buildVotePrompt(conflict: Conflict): string {
    return `
请作为评审专家，对以下冲突进行投票。
...
`;
   ...
   * 从投票结果中提取胜出者
   */
  private extractWinnerFromVote(result: string, candidates: string[]): string | null {
   ...
   * 记录到知识中心
   */
  private recordToKnowledgeCenter(conflict: Conflict, resolution: ConflictResolution): void {
