...
export function setKanbanDatabase(db: Database) {
  _db = db;
}

function getDb(): Database {
  if (!_db) throw new Error('Kanban database not initialized. Call setKanbanDatabase() first.');
  return _db;
}
    ...
    defineTool({
      ...
      execute: async (input, _context) => {
        try {
          ...
          const values: (string | number)[] = [input.status];
