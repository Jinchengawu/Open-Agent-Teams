...
export interface EvaluationResult {
  agentId: string;
  taskType: string;
  scores: Record<EvaluationDimension, number>; // 1-10
  overallScore: number;
  feedback: string;
  suggestions: string[];
  timestamp: string;
...
export class OutputJudge {
  ...
  async evaluate(request: EvaluationRequest): Promise<EvaluationResult> {
    ...
    try {
      ...
      const fallback: EvaluationResult = {
        ...
        scores: Object.fromEntries(dimensions.map(d => [d, 5])) as Record<EvaluationDimension, number>,
        overallScore: 5,
  ...
  /** 构建评估提示词 */
  private buildPrompt(request: EvaluationRequest, dimensions: EvaluationDimension[]): string {
...
请以 JSON 格式返回评估结果：
{
  ...
  /** 获取维度描述 */
  private getDimensionDescription(dim: EvaluationDimension): string {
    const descriptions: Record<EvaluationDimension, string> = {
  ...
  private parseResponse(response: string, agentId: string, taskType: string): EvaluationResult {
    try {
      ...
      return {
        agentId,
        taskType,
        scores: Object.fromEntries(DEFAULT_DIMENSIONS.map(d => [d, 5])) as Record<EvaluationDimension, number>,
        overallScore: 5,
        feedback: '响应解析失败',
        suggestions: [],
        timestamp: new Date().toISOString(),
