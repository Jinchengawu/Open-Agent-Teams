...
export function createA2ATextPart(text: string, metadata?: Record<string, unknown>): A2APart {
  return { kind: 'text', text, metadata };
}

export function createA2ADataPart(data: Record<string, unknown>, metadata?: Record<string, unknown>): A2APart {
...
export function agentMessageToA2AMessage(message: AgentMessage): A2AMessage {
  return {
...
export function a2aMessageToAgentMessage(message: A2AMessage, fallback: {
  from: string;
  to: string | 'broadcast';
  type?: AgentMessage['type'];
}): Omit<AgentMessage, 'id' | 'metadata'> & { metadata?: Partial<AgentMessage['metadata']> } {
  return {
    from: String(message.metadata?.from || fallback.from),
    to: String(message.metadata?.to || fallback.to) as string | 'broadcast',
...
export function teamProfileAgentToA2AAgentCard(profile: TeamProfile, agent: TeamProfileAgentDefinition): A2AAgentCard {
...
export function teamProfileToA2AAgentCards(profile: TeamProfile): A2AAgentCard[] {
  return profile.agents.map((agent) => teamProfileAgentToA2AAgentCard(profile, agent));
}

export function pipelineInstanceToA2ATask(instance: PipelineInstance): A2ATask {
  ...
  return {
    ...
    artifacts,
    history: Array.from(instance.surfaceResults.values()).map((result) =>
      createA2AMessage({
        role: 'agent',
        contextId,
        taskId: instance.id,
...
export function surfaceResultToA2AArtifact(result: SurfaceResult): A2AArtifact {
  return {
...
export function pipelineStatusToA2AState(status: PipelineStatus): A2ATaskState {
  switch (status) {
    case 'pending':
      return 'submitted';
    case 'running':
...
export function surfaceStatusToA2AState(status: SurfaceStatus): A2ATaskState {
  switch (status) {
    case 'pending':
    case 'waiting':
      return 'submitted';
...
function inferAgentMessageType(message: A2AMessage): AgentMessage['type'] {
  const kind = String(message.metadata?.kind || message.metadata?.type || '').toLowerCase();
  if (kind === 'task' || kind === 'task_assign') return 'task_assign';
  if (kind === 'status' || kind === 'status_update') return 'status_update';
  if (kind === 'tool' || kind === 'tool_call') return 'tool_call';
  if (kind === 'meeting' || kind === 'meeting_round') return 'meeting_round';
  return 'chat';
}
