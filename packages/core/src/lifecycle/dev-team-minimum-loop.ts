 ...
 */
export const DEV_TEAM_MINIMUM_LOOP_PIPELINE: PipelineDefinition = {
  id: 'dev-team-minimum-loop',
  name: 'Agent Team Minimum Lifecycle',
  version: '0.1.0',
  context: {
    description:
      'A narrow software delivery loop that turns one user request into documents, tasks, implementation guidance, verification, release notes, and experience capture.',
    cache: {
      compression: true,
      retention: '14d',
  ...
  surfaces: [
    {
      ...
      workflow: {
        goal:
          'Lead the requirement discovery surface for the user request. Clarify goals, users, constraints, risks, acceptance criteria, and produce a PRD-style markdown document. If document tools are available, create or update a document so the result can be reused by downstream surfaces.',
        steps: [
        ...
        context:
          'This surface represents meeting mode as a durable output: the expected result is a decision-bearing document, not a transcript.',
      },
      gate: { type: 'auto' },
    },
    {
      ...
      workflow: {
        goal:
          'Convert the PRD into a Kanban-ready execution plan. Split the work into small tasks, assign each task to the best Role Agent, define status/progress expectations, and bind each task to the document and lifecycle stage. If Kanban tools are available, create the initial tasks.',
        steps: [
        ...
        context:
          'Kanban is the team state projection. It should connect documents, tasks, owners, workflow stages, blockers, and artifacts.',
      },
      gate: { type: 'auto' },
    },
    {
      ...
      workflow: {
        goal:
          'Execute the frontend portion of the task plan. Produce implementation notes, code where appropriate, affected files, test expectations, and any blockers.',
        steps: [
    ...
    },
    {
      ...
      workflow: {
        goal:
          'Execute the backend portion of the task plan. Produce API/data design, implementation notes, code where appropriate, test expectations, and any blockers.',
        steps: [
          'Read the PRD and task plan from upstream artifacts.',
    ...
    },
    {
      ...
      workflow: {
        goal:
          'Verify the implementation artifacts from frontend and backend surfaces. Produce a test plan, recommended commands, expected evidence, and a release risk assessment.',
        steps: [
    ...
    },
    {
      ...
      workflow: {
        goal:
          'Prepare release readiness for the verified work. Produce deployment steps, rollback notes, environment assumptions, monitoring checks, and release notes.',
        steps: [
    ...
    },
    {
      ...
      workflow: {
        goal:
          'Capture what the team learned from this lifecycle run. Summarize decisions, reusable patterns, unresolved risks, follow-up tasks, and knowledge that should be saved for future work.',
        steps: [
        ...
        context:
          'Experience is the durable learning extracted from work. It should be saved as a document or knowledge artifact.',
      },
      gate: { type: 'auto' },
    },
  ],
