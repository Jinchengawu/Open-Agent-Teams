...
import type { MeetingEvent, KanbanEvent } from '../types.js';
 ...
 * 注册会议事件处理器
 */
export function registerMeetingHandlers(deps: MeetingHandlerDeps = {}): void {
  const { createKanbanTask, saveMeetingDocument } = deps;

  // ── 会议完成 → 自动创建看板任务 ──
  eventBus.on('meeting.completed', async (event: MeetingEvent) => {
  ...
  // ── 会议轮次完成 → 日志记录 ──
  eventBus.on('meeting.round_completed', async (event: MeetingEvent) => {
