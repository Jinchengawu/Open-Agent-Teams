...
import type { WorkflowEvent, KanbanEvent } from '../types.js';
...
export interface WorkflowHandlerDeps {
  ...
  /** 获取任务绑定的看板任务 */
  getTaskByWorkflowId?: (workflowId: string) => Promise<{ taskId: string } | null>;
}

/**
 * 注册工作流事件处理器
 */
export function registerWorkflowHandlers(deps: WorkflowHandlerDeps = {}): void {
  const { updateTaskStatus, updateTaskProgress, getTaskByWorkflowId } = deps;

  // ── 工作流完成 → 更新看板任务状态 ──
  eventBus.on('workflow.completed', async (event: WorkflowEvent) => {
  ...
  eventBus.on('workflow.failed', async (event: WorkflowEvent) => {
  ...
  // ── 工作流步骤完成 → 更新进度 ──
  eventBus.on('workflow.step_completed', async (event: WorkflowEvent) => {
    const { workflowId, taskId, stepIndex, totalSteps } = event.payload;
    const targetTaskId = taskId || (await getTaskByWorkflowId?.(workflowId))?.taskId;

    if (!targetTaskId || stepIndex === undefined || totalSteps === undefined) return;
  ...
  eventBus.on('workflow.started', async (event: WorkflowEvent) => {
