...
import type { KanbanEvent, WorkflowEvent } from '../types.js';

export interface KanbanHandlerDeps {
  /** 触发工作流执行 */
  runWorkflow?: (taskId: string, goal: string) => Promise<void>;
  /** 获取任务详情 */
  getTask?: (taskId: string) => Promise<{ title: string; description: string; assignee?: string } | null>;
  /** 分配任务给 Agent */
...
export function registerKanbanHandlers(deps: KanbanHandlerDeps = {}): void {
  ...
  // ── 看板任务状态变更 → 触发工作流 ──
  eventBus.on('kanban.task.status_changed', async (event: KanbanEvent) => {
  ...
  // ── 看板任务创建 → 自动分配 Agent ──
  eventBus.on('kanban.task.created', async (event: KanbanEvent) => {
