...
export class ProjectMemory {
  ...
  get(key: string): MemoryEntry | null {
    return (this.db.prepare('SELECT * FROM project_memory WHERE key = ?').get(key) as MemoryEntry) ?? null;
  }

  /** 获取记忆条目（按 id） */
  getById(id: number): MemoryEntry | null {
    return (this.db.prepare('SELECT * FROM project_memory WHERE id = ?').get(id) as MemoryEntry) ?? null;
  }
  ...
  query(options: MemoryQuery): MemoryEntry[] {
    ...
    if (search && this.ftsAvailable) {
      ...
      return this.db.prepare(`
        ...
        LIMIT ? OFFSET ?
      `).all(
        ...(key ? [search, key, limit, offset] : [search, limit, offset])
      ) as MemoryEntry[];
    }
    ...
    if (search) {
      ...
      return this.db.prepare(`
        ...
        LIMIT ? OFFSET ?
      `).all(
        ...(key ? [pattern, pattern, pattern, key, limit, offset] : [pattern, pattern, pattern, limit, offset])
      ) as MemoryEntry[];
    }
    ...
    return this.db.prepare(`
      ...
      ORDER BY updated_at DESC
      LIMIT ? OFFSET ?
    `).all(...(key ? [key, limit, offset] : [limit, offset])) as MemoryEntry[];
  }

  /** 获取历史版本 */
  getVersions(key: string): Array<{ value: string; metadata: string; version: number; created_at: string }> {
    ...
    `).all(entry.id) as Array<{ value: string; metadata: string; version: number; created_at: string }>;
  }
  ...
  /** 获取条目数量 */
  count(): number {
    const row = this.db.prepare('SELECT COUNT(*) as count FROM project_memory').get() as { count: number };
    return row.count;
  }
  ...
  buildContext(keys?: string[]): string {
    ...
    if (keys && keys.length > 0) {
      entries = keys.map(k => this.get(k)).filter(Boolean) as MemoryEntry[];
    } else {
