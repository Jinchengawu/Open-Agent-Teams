...
export class MemoryStore {
  ...
  set(agentId: string, key: string, value: string, ttlMs?: number): void {
    ...
    this.db
      .prepare(
        `INSERT INTO agent_memory (agent_id, key, value, ttl)
         VALUES (?, ?, ?, ?)
  ...
  get(agentId: string, key: string): string | null {
    const row = this.db
      .prepare(
        'SELECT value, ttl FROM agent_memory WHERE agent_id = ? AND key = ?'
      )
      .get(agentId, key) as { value: string; ttl: number | null } | undefined;
    if (!row) return null;
  ...
  search(agentId: string, query: string): { key: string; value: string }[] {
    const pattern = `%${query}%`;
    return this.db
      .prepare(
      ...
      .all(agentId, pattern, pattern, Date.now()) as {
      key: string;
      value: string;
    }[];
  }
  ...
  keys(agentId: string): string[] {
    const rows = this.db
      ...
      .all(agentId, Date.now()) as { key: string }[];
